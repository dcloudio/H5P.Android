...
import io.dcloud.media.video.ijkplayer.danmaku.BaseDanmakuConverter;
import io.dcloud.media.video.ijkplayer.danmaku.BiliDanmukuParser;
import io.dcloud.media.video.ijkplayer.danmaku.OnDanmakuListener;
import io.dcloud.media.video.ijkplayer.danmaku.StandardDanmaKuParser;
import io.dcloud.media.video.ijkplayer.utils.MotionEventUtils;
import io.dcloud.media.video.ijkplayer.utils.NavUtils;
...
import static io.dcloud.media.video.ijkplayer.utils.StringUtils.generateTime;
...
//                mLlBottomBar.setVisibility(View.VISIBLE);
     ...
     * @param isShow
     */
    public void setCenterPlayBntVisibility(boolean isShow) {
    ...
    public void onClick(View v) {
        ...
        /*if (id == VideoR.VIDEO_IJK_ID_IV_BACK) {
            onBackPressed();
        } else*/ if (id == VideoR.VIDEO_IJK_ID_IV_PLAY || id == VideoR.VIDEO_IJK_ID_IV_PLAY_CIRCLE) {
            _togglePlayStatus();
...
//            mAttachActivity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
    ...
    String[] rates = new String[]{"0.5","0.8","1.0","1.25","1.5"};
...
//    private RadioGroup mDanmakuColorOptions;
    ...
    private void _initDanmaku() {
        // 弹幕控制
        mDanmakuView = (IDanmakuView) findViewById(VideoR.VIDEO_IJK_ID_SV_DANMAKU);
...
//            mDanmakuContext.setDanmakuSync(new VideoDanmakuSync(this));
     ...
     * @return
     */
    public IjkPlayerView enableDanmuBtn(boolean isEnable) {
        if(mIsEnableDanmaku){
            if (isEnable) {
